	...
	<div>
		<ul class="myul" :style="{width: (barNum.length * 40) + 'px'}">
			...
			<li v-if="barNum && barNum.indexOf(2) !== -1">
				<div class="myBtn" @click="openAddTow()" title="每个模块生成同现场调查相同数量的行">A</div>
			</li>
			...
			<li v-if="barNum && barNum.indexOf(4) !== -1">
				<div class="myBtn" @click="copyRow('copyAll')" title="复制全部数据">C</div>
			</li>
			...
			<li v-if="barNum && barNum.indexOf(6) !== -1" title="生成重复样" @click="addSample(index)">
				<div class="___relative">
					<el-popover placement="top-start" title="生成重复样" width="320" trigger="click">
...
<script>
  ...
  export default {
    ...
    methods: {
      //添加行
      increase() {
        let obj = JSON.parse(JSON.stringify(this.data.valueData.point[this.index]));
      ...
      reduce() {
        ...
        this.jsonString.forEach((item, index) => {
          pointArr.push(...item.data.valueData.point.filter((val, num) => val.sampleNum === sampleNum))
        });
        if (sampleNum !== '' && sampleNum !== undefined && pointArr.length === 1) {
      ...
      resetNum() {
        let dataArr = [];
        this.jsonString.forEach((item, index) => {
          if (item.data.name === this.data.name && item.data.valueData.multipleId === this.data.valueData.multipleId) {
        ...
        sampleNumArr.forEach((item, index) => {
          integrationArr.push(dataArr.filter((val, num) => val.sampleNum === item))
        });
      ...
      addTwo() {
        ...
        if (this.jsonString) {
          ...
          let nameArr = Array.from(
                  new Set(
                          this.jsonString.map(j => j.to + "..." + j.data.valueData.multipleId)
                  )
          );
