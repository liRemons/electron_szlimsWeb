  ...
  methods: {
    testShow() {
      ...
      if (this.$route.query.isEdit == true) {
        ...
        this.onlyRead = this.$route.query.solutionPass !== 0;

        newSolutionPreparationData = JSON.myParse(
          newSolutionPreparationDataStr
        );
        contentArray = newSolutionPreparationData.map(
    ...
    redefinition(needPorjectName) {
      ...
      this.jsonString.forEach((item, index) => {
        let obj = {};
        let findIndex = redefinitionArr.findIndex(
          (val) =>
            val.to === item.to &&
      ...
      redefinitionArr.forEach((item, index) => {
        ...
        if (item.data.height._normal.confirm) {
          ...
          let several = Math.floor(
            (surplusHiehgt - projcet.data.height._normal.fixed) / 32
          );
          if (several > 2) {
            several -= 2;
          }
