...
export default {
  ...
  methods: {
    ...
    showExample() {
        ...
        if (result1) {
          ...
          if (result2) {
            ...
            if (this.task.data != null) {
              ...
              result2.valueData = this.headData(this.task, contentArray[i].name)
            } else {
              result2.valueData = this.headData(this.task, contentArray[i].name)
      ...
      if (!this.task.data) {
        if (this.task.specifications === '射频+工频') {
          obj = obj.filter((item) => item.name !== 'project_dc_dchjcl')
        }
      }
    ...
    operation() {
      ...
      this.testContentArray.forEach((item, index) => {
        ...
        if (
          this.task.specifications === '射频+工频' &&
          item.testProjectName === 'project_dc'
        ) {
          this.control[index] = true
        }
        let del = this.__getPoint(this.jsonString, 'project_deleteReason')

        if (
          del.length &&
          del.find((el) => item.testProjectChineseName === el.testprojectName)
        ) {
    ...
    changeModule(obj, index) {
      ...
      if (this.control[index] === false) {
        ...
        // 如果是电磁========START
        if (['project_dc', 'project_dc_gp'].includes(obj.testProjectName)) {
          if (
            obj.testProjectName === 'project_dc_gp' ||
            this.task.specifications === '射频+工频'
          ) {
            this.jsonString = this.jsonString.filter(
              (item) => item.to !== 'project_dc_gpspjcdxxx'
            )
            cancel = ['project_dc_gpspjcdxxx']
          }
          ...
          if (Index > 0) {
            this.jsonString.splice(Index, 0, ...createArr)
          } else {
        ...
        modelName.forEach((item) => {
          ...
          if (result) {
            ...
            result.valueData.multipleId = Math.random() + 'init' + Math.random()
  ...
  mounted() {
    ...
    if (this.unitUrl) {
      ...
      getImageBase64Data(this.hostUrl + this.taskData.unitUrl).then((res) => {
        this.taskData.showing[0][0]['data']['valueData']['imgBase64Three'] = res
      })
    }
    // try {
