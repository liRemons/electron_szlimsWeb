...
function Draw(canvas, degree, config = {}) {
    ...
    const point = {};
    const isIE = ((!!window.ActiveXObject || "ActiveXObject" in window) ? true : false);
    const isMobile = /phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone/i.test(navigator.userAgent);
    console.log("isIE:", isIE, "   isMobile:", isMobile)
    ...
    const create = signal => function(e) {
        e.preventDefault();
        if (signal === 1) {
            pressed = true;
        }
        if (signal === 1 || pressed) {
            if (e.type == "touchstart" || e.type == "touchmove") e = e.touches[0]; //如果isMobile为true 就可以手写
    ...
    if (isMobile) {
        ...
        canvas.addEventListener('pointermove', optimizedMove);
        ['mouseup', 'mouseleave', 'touchend', 'pointerup'].forEach((event) => {
...
Draw.prototype = {
    ...
    upload(blob, url, success, failure, data) {
