		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading[0]" :defaultValue="item.reading[0]"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading[1]" :defaultValue="item.reading[1]"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading[2]" :defaultValue="item.reading[2]"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading[3]" :defaultValue="item.reading[3]"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading[4]" :defaultValue="item.reading[4]"
										 @change.native="changeNum(index)"></myInput>
						<div class="___absolute toolBar" v-if="false" style="left: 580px;top:0; width: 62px;">
					...
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						...
						<div class="___absolute toolBar" style="left: 200px;top:0; width: 180px;" v-if="target ==0">
    ...
    methods: {
      getIdPoint(keyArr, id) {
        this.jsonString.forEach((item, index) => {
          if (item.data.name === "project_jc_xczd5" && this.data.valueData.multipleId === item.data.valueData.multipleId) {
      ...
      detectionResult(heBingId) {
        ...
        let retain = [];
        dataArr.forEach((item, index) => {
          let average = this.average(item.reading).toFixed46(this.size(item.reading));
          ...
          if (this.isNumber(average)) {
            ...
            if (this.isNumber(item.correction)) {
              value = algorithm === 1 ? reading * correction : reading + correction;
            } else {
              value = reading;
            }
            cumulative += value
        ...
        if (this.isNumber(cumulative)) {
          ...
          dataArr.forEach((item, index) => {
            if (item.heBingId === heBingId) {
              dataArr[index].result = lastNum.toFixed46(this.sizeMin(retain), false, true);
            }
          })
        }
      },
      ...
      changeNum(index) {
        if (this.getIdenticalPoint(this.data.valueData.point[index].heBingId).every((item, index, array) => {
          ...
          if (this.data.valueData.point[index].factorArr !== '' && this.data.valueData.point[index].method !== '' && this.isNumber(average)) {
            ...
            let factorArr = this.decompose(factorObj, this.data.valueData.point[index].method);
            let correction = '';
            ...
            this.data.valueData.point[index].correction = parseFloat(correction).toFixed46(this.computeObj.size([factorObj[0].Amendment]));
          } else {
      ...
      addRow(index, Judge, heBingId) {
        ...
        if (Judge === 1) {
          ...
          row.reading = ['', '', '', '', ''];
          row.correction = '';
          let id = 'project_jc_xczd5-' + this.pageNumber + '-' + this.thisPageIndex + '-' + index;
          ...
          let id = 'project_jc_xczd5-' + this.pageNumber + '-' + this.thisPageIndex;
      ...
      sureSampleNum() {
        ...
        if (this.selectItem.sampleQuantity == 1) {
          ...
          for (let i = 0; i < this.selectItem.sampleQuantity; i++) {
            ...
            let id = 'project_jc_xczd5-' + this.pageNumber + '-' + this.thisPageIndex + '-' + (this.data.valueData.point.length + i);
