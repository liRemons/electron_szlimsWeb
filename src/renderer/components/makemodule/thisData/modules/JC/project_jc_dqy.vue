		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.temperature" @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.meterVal" @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<div>{{item.scaleVal}}</div>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.temperatureCoefficient"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.supplementVal"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<div>{{item.result}}</div>
					...
					<td class="___relative tc">
						...
						<div class="___absolute toolBar" style="left: 157px;top:0; width: 180px;">
  ...
  export default {
    ...
    methods: {
      ...
      getResultAverageArr(sampleNum) {
        if (sampleNum) {
          ...
          this.jsonString.forEach((item, index) => {
            if (item.data.name === "project_jc_dqy" && this.data.valueData.multipleId === item.data.valueData.multipleId) {
      ...
      changeNum(index) {
        ...
        // supplementVal
        // result
        let temperature = parseFloat(this.data.valueData.point[index].temperature);
        let meterVal = parseFloat(this.data.valueData.point[index].meterVal);
        let temperatureCoefficient = parseFloat(this.data.valueData.point[index].temperatureCoefficient);
        let supplementVal = parseFloat(this.data.valueData.point[index].supplementVal);
        if (this.data.valueData.point[index].factorArr !== '' && this.data.valueData.point[index].method !== '' && this.isNumber(meterVal)) {
          ...
          if (this.data.valueData.point[index].method === 1) {
            correction = this.getFactor(this.data.valueData.point[index].meterVal, factorArr)
          } else {
            correction = this.getRange(this.data.valueData.point[index].meterVal, factorArr)
          }
          this.data.valueData.point[index].scaleVal = correction;
          let scaleVal = parseFloat(correction);
          if (this.isNumber(temperature) && this.isNumber(meterVal) && this.isNumber(temperatureCoefficient) && this.isNumber(supplementVal)) {
            let P2 = temperatureCoefficient * temperature;
            this.data.valueData.point[index].result = parseFloat(meterVal + scaleVal + P2 + supplementVal).toFixed46(1, false, true);
