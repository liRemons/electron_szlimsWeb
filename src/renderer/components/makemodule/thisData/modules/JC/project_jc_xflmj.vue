		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					<td v-if="item.noShow" :rowspan="item.heBingLength" @click="generateSampleNum(data.valueData.point)">
					...
					<td class="___relative">
						<div class="___relative tl" style="line-height: 24px;">
							...
							<myInput style="text-align: center;width:110px;height: 24px;" v-model="item.position"
											 @change.native=""></myInput>
						</div>
						<div class="___relative tl" style="line-height: 24px;">
							<span class="ml5">形状：</span>
							<div class="___absolute" style="width: 110px;left:45px;top:1px;">
								<selectModel @returnVal="returnVal5"
														 style="height:24px;"
														 :Judge="true"
														 :special="index"
						...
						<div class="___relative tl" style="line-height: 24px;height: 24px;">
							<div v-if="item.shape==='矩形'" class="___relative">
								...
								<myInput class="___relative" style="width:50px;text-align: center;height: 24px;top:1px;"
												 v-model="item.formula[0]" @change.native="getMeasureArea(index)"
												 :defaultValue="item.formula[0]"></myInput>
								<span>x</span>
								<myInput class="___relative" style="width:50px;text-align: center;height: 24px;top:1px;"
												 v-model="item.formula[1]" @change.native="getMeasureArea(index)"
												 :defaultValue="item.formula[1]"></myInput>
								<span>m</span>
							</div>
							<div v-if="item.shape==='圆形'" class="___relative">
								<span class="ml5">直径：</span>
								<myInput class="___relative" style="width:80px;text-align: center;height: 24px;top:1px;"
												 v-model="item.formula[0]" @change.native="getMeasureArea(index)"
												 :defaultValue="item.formula[0]"></myInput>
								<span>m</span>
							</div>
						...
						<div class="___relative tl" style="line-height: 24px;">
							...
							<div style="width:95px;left:40px;" class="___absolute t0 tc">{{item.measureArea}}</div>
							<span>m²</span>
						</div>
					</td>
					<td class="___relative">
						<div class="___relative"
							...
							<div v-for="(v,i) in 4"
									 ...
									 :style="{left:(86*i)+'px'}"
									 style="width:86px;line-height: 24px;">
								<div class="___relative borderRight" style="width:30px;">
									<span>{{(i+1)+(num*4)}}</span>
								</div>
								<div class="___absolute t0" style="width:56px;left:30px;line-height: 24px;">
									<myInput style="text-align: center;width:54px;height: 24px;" v-model="item.windSpeed[i+(num*4)]"
					...
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						<myInput style="text-align: center" v-model="item.number"
										 :defaultValue="item.number"
										 @change.native="changePeople(index,item.number,item.heBingId)"></myInput>
					</td>
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
					...
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						<div class="___relative">
							...
							<div class="___relative" style="line-height: 16px;">
								<querySelect v-model="item.tuyereNum"
														 style="height:16px;"
														 v-if="target === '0'"
														 ref="querySelect"
														 :num="index"
							...
							<div class="___relative" style="line-height: 16px;">
								<querySelect v-model="item.regionNum"
														 style="height:16px;"
														 v-if="target === '0'"
														 ref="querySelect"
														 :num="index"
														 :list="deviceData"
														 :defaultValue="item.regionNum"
														 @getSelectItem="returnVal4"
														 :name="'deviceNum'">
								</querySelect>
								<div v-else>
									{{item.tuyereNum}}
							...
							<div class="___relative" style="line-height: 16px;">
								<querySelect v-model="item.windSpeedNum"
														 style="height:16px;"
														 v-if="target === '0'"
														 ref="querySelect"
														 :num="index"
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="样品数量">
							<el-input placeholder="请输入样品数量" @keyup.native="myCaiSampleNum = myCaiSampleNum.replace(/[^\d]/g,'')"
												:disabled="myCaiSample !== '其它' && myCaiSample !== '均匀布点'"
												v-model="myCaiSampleNum"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
			<div style="text-align: right; margin-right: 50px;">
				<el-button style="margin-top: 15px; margin-left:470px;" type="primary" @click="sureSampleNum">确定</el-button>
			</div>
		</el-dialog>
	</div>
</template>
  ...
  export default {
    data() {
      return {
        ...
        myCaiSample: "",
        myCaiSampleNum: "",
        factorList: ['中央布点', '对称布点', '对角线布点', '梅花布点', '均匀布点', '其它'],
        myggcspoint: []
      }
    },
    computed: {},
    ...
    methods: {
      getIdPoint(keyArr, id) {
        this.jsonString.forEach((item, index) => {
          if (item.data.name === "project_jc_xflmj" && this.data.valueData.multipleId === item.data.valueData.multipleId) {
      ...
      detectionResult(heBingId) {
        ...
        dataArr.forEach((item, index) => {
          ...
          if (this.isNumber(average) && this.isNumber(item.correction) && this.isNumber(item.measureArea)) {
        ...
        if (this.isNumber(val1)) {
          if (this.isNumber(val2)) {
            if (this.data.valueData.point[index].shape === "矩形") {
              this.data.valueData.point[index].measureArea = (val1 * val2).toFixed46(3);
            }
          }
          if (this.data.valueData.point[index].shape === "圆形") {
            let radius = val1 / 2;
            this.data.valueData.point[index].measureArea = (Math.PI * (radius * radius)).toFixed46(3);
          }
        } else {
      ...
      changeNum(index) {
        ...
        if (this.data.valueData.point[index].factorArr !== '' && this.data.valueData.point[index].method !== '' && this.isNumber(average)) {
          let factorArr = this.decompose(this.data.valueData.point[index].factorArr, this.data.valueData.point[index].method);
          let correction = '';
      ...
      returnVal2(index, val) {
        if (val.deviceFactor != null && val.calibrationMethod != null) {
          ...
          let deviceNum = '';
          let algorithm = '';
          if (val.calibrationMethod === '测量值/修正值' || val.calibrationMethod === '测量值/修正因子') {
            method = 1;
          } else if (val.calibrationMethod === '测量范围/修正值' || val.calibrationMethod === '测量范围/修正因子') {
            method = 2;
          }
          if (val.calibrationMethod === '测量值/修正因子' || val.calibrationMethod === '测量范围/修正因子') {
            algorithm = 1;
          } else if (val.calibrationMethod === '测量值/修正值' || val.calibrationMethod === '测量范围/修正值') {
            algorithm = 2;
          }
          SampleTools = val.id;
      ...
      addRow(index, Judge, heBingId) {
        ...
        if (Judge === 1) {
          ...
          row.measureArea = '';
          row.windSpeed = ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''];
          let id = 'project_jc_xflmj-' + this.pageNumber + '-' + this.thisPageIndex + '-' + index;
          ...
          let id = 'project_jc_xflmj-' + this.pageNumber + '-' + this.thisPageIndex;
      ...
      sureSampleNum() {
        ...
        if (this.selectItem.sampleQuantity == 1) {
          ...
          let index = this.data.valueData.point.findIndex((item, index) => item.foreverId === obj.foreverId);
          this.clearPingXing();
          ...
          for (let i = 0; i < this.selectItem.sampleQuantity; i++) {
            ...
            let id = 'project_jc_xflmj-' + this.pageNumber + '-' + this.thisPageIndex + '-' + (this.data.valueData.point.length + i);
