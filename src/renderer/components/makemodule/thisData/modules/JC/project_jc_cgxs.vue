		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						<myInput style="text-align: center" v-model="item.lengthGroundA"
										 :defaultValue="item.lengthGroundA"
										 @change.native="changeVal(index,'lengthGroundA',item.lengthGroundA,item.heBingId)"></myInput>
					</td>
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						<myInput style="text-align: center" v-model="item.lengthGroundB"
										 :defaultValue="item.lengthGroundB"
										 @change.native="changeVal(index,'lengthGroundB',item.lengthGroundB,item.heBingId)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.windowLengthA" :defaultValue="item.windowLengthA"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.windowLengthB" :defaultValue="item.windowLengthB"
										 @change.native="changeNum(index)"></myInput>
					</td>
					<td class="___relative tc">
    ...
    methods: {
      getIdPoint(keyArr, id) {
        this.jsonString.forEach((item, index) => {
          if (item.data.name === "project_jc_cgxs" && this.data.valueData.multipleId === item.data.valueData.multipleId) {
      ...
      detectionResult(heBingId) {
        ...
        if (this.isNumber(cumulative) && this.isNumber(lengthGroundA) && this.isNumber(lengthGroundB)) {
      ...
      addRow(index, Judge, heBingId) {
        ...
        if (Judge === 1) {
          ...
          row.windowLengthB = '';
          row.windowNum = '';
          let id = 'project_jc_cgxs-' + this.pageNumber + '-' + this.thisPageIndex + '-' + index;
          ...
          let id = 'project_jc_cgxs-' + this.pageNumber + '-' + this.thisPageIndex;
      ...
      sureSampleNum() {
        ...
        if (this.selectItem.sampleQuantity == 1) {
          ...
          for (let i = 0; i < this.selectItem.sampleQuantity; i++) {
            ...
            let id = 'project_jc_cgxs-' + this.pageNumber + '-' + this.thisPageIndex + '-' + (this.data.valueData.point.length + i);
