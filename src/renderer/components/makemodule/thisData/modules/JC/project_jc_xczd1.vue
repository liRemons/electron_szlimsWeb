	...
	<div>
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<div style="border: 1px solid black; line-height: 32px; text-align: left; padding-left: 10px; border-top: none;border-bottom:none">
				检测项目：{{data.valueData.testProjectChineseName}}{{data.valueData.detectionLimitPieces}} &nbsp;&nbsp;&nbsp;&nbsp;
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td class="___relative tc">
						<myInput style="text-align: center" v-model="item.reading" :defaultValue="item.reading"
										 @change.native="changeNum(index)"></myInput>
						<div class="___absolute toolBar" v-if="target === '0'" style="left: 763px;top:0; width: 62px;">
					...
					<td v-if="item.noShow" class="___relative tc" :rowspan="item.heBingLength">
						...
						<div class="___absolute toolBar" style="left: 240px;top:0; width: 180px;">
    ...
    methods: {
      getIdPoint(keyArr, id) {
        this.jsonString.forEach((item, index) => {
          if (item.data.name === "project_jc_xczd1" && this.data.valueData.multipleId === item.data.valueData.multipleId) {
      ...
      changeNum(index) {
        if (this.getIdenticalPoint(this.data.valueData.point[index].heBingId).every((item, index, array) => {
          ...
          if (this.data.valueData.point[index].factorArr !== '' && this.data.valueData.point[index].method !== '') {
            ...
            if (this.data.valueData.point[index].method === 1) {
              correction = this.getFactor(this.data.valueData.point[index].reading, factorArr)
            } else {
              correction = this.getRange(this.data.valueData.point[index].reading, factorArr)
            }
      ...
      addRow(index, Judge, heBingId) {
        ...
        if (Judge === 1) {
          ...
          row.reading = "";
          row.correction = "";
          let id = "project_jc_xczd1-" + this.pageNumber + "-" + this.thisPageIndex + "-" + index;
          ...
          let id = "project_jc_xczd1-" + this.pageNumber + "-" + this.thisPageIndex;
      ...
      sureSampleNum() {
        ...
        if (this.selectItem.sampleQuantity == 1) {
          ...
          for (let i = 0; i < this.selectItem.sampleQuantity; i++) {
            ...
            let id = 'project_jc_xczd1-' + this.pageNumber + '-' + this.thisPageIndex + '-' + (this.data.valueData.point.length + i);
