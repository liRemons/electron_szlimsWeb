<template>
    <div :class="{_normalHeight_:true}" class="___relative" v-if="data.height._normal.carried" :id="data.valueData.testProjectId">
        <div :class="{eventCover:!ableInput}"></div>
        <div class="modules_1_tableBox ___relative mt20">
            <div style="border: 1px solid black;width: 710px;height: 32px; line-height: 32px;" class="tc">
                <p>机房楼上和楼下关注点情况</p>
            </div>
            <div v-show="show" class="___absolute  el-icon-arrow-down" style="left: 760px; top: 0px; color:gray; font-size: 28px; cursor: pointer; width: 50px; height: 50px;" @click="toFold"></div>
            <div v-show="!show" class="___absolute el-icon-arrow-right" style="left: 760px; top: 0px; color:gray; font-size: 28px; cursor: pointer; width: 50px; height: 50px;" @click="toNoFold"></div>
            <div v-if="showList">
                <div class="___relative ___module_frame_Box" style="border-top: solid 1px black;">
                    <div class="___relative">
                        ...
                        <div style="width:180px;left:80px;" class="tc borderRight ___absolute t0 Full">
                            <span>名称</span>
                        </div>
                        <div style="width:150px;left:260px;" class="tc borderRight ___absolute t0 Full">
                            <span>关注点</span>
                        </div>
                        <div style="width:200px;left:410px;" class="tc borderRight ___absolute t0 Full">
                            <span>毗邻场所</span>
                        </div>
                    ...
                    <div class="___relative" :class="index!=data.valueData.point.length-1 ? 'borderBottom':''"
                        ...
                        <div style="width:200px;left:410px; line-height: 16px;" class="tc borderRight ___absolute t0 Full">
                            <divModel v-model="item.rows[2]" style="width:100%;text-align: center;"
                                      class="Full warp2 rowsInput2 hide focusBg"></divModel>
                        </div>
                        <div style="width:100px;right: 0" @click="getIndex(index)" class="tc ___absolute t0 Full">
                        ...
                        <div class="__functionBox" v-if="index!=data.valueData.point.length-1&&ipdTemplate=='ipdTemplate' && target == 0"
                ...
                <div class="__functionBox" style="right:-155px;width: 110px;" v-if="ipdTemplate=='ipdTemplate' && target == 0">
                    <!-- ******** 功能模块 ********** -->
                    <div class="__functionButton6" @dblclick="reduce(data.valueData.point.length-1)" style="left:5px;">
                        <span>-</span>
                    </div>
                    <div class="__functionButton6" @click="sure" style="left:30px;font-size: 14px;">
                        <span class="el-icon-search"></span>
                    </div>
                    <div class="__functionButton6" @click="increase" style="left: 60px;">
                        <span>+</span>
                    </div>
                    <div class="__functionButton6" title="清除数据"  @dblclick="computeObj.deleteRow('actionsbuildingArr')">
                        <span>d</span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        data() {
            return {
                patternIndex: "",
                factorList: ['1', '0.25', '0.5', '0.2', '0.125', '0.0625', '0.05', '0.025'],
                namArr:['机房上方', '机房下方'],
                showList: true,
                show: true
            }
                ...
                if (nameArr.every((item) => item != '')) {
                    ...
                    if (Array.from(set).length < nameArr.length) {
                        ...
                        dataArr.forEach((item, index) => {
                            ...
                            let obj = [item.rows[0] + '(' + remarks + ')', item.rows[2], item.rows[3], '', '', '', '', '', '', item.myId];
