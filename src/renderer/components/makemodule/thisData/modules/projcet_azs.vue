<template>
  <div class="___relative _normalHeight_">
    ...
    <div class="modules_1_tableBox ___relative mt10">
      ...
      >
        <p class="ml5 tl">
          验收：像素值或DDI在规定值内，或建立基线值，影像均匀无伪影；状态&稳定性：像素值或DDI在规定值内，或基线值<br />±50.0%，影像均匀无伪影。
        </p>
      </div>
      ...
      >
        <p class="ml5">
          有可能的话取出滤线栅，关闭遮线器，用面积15cm×15cm，厚2mm铅板覆盖出线口，设置最低管电流积或最低管电流和<br />最低管电压进行手动曝光并获取一幅空白影像，读取影像中心10cm×10cmROI的平均像素值或记录DDI值。调节适当窗宽<br />窗位，目视检查影像应均匀无伪影。
        </p>
      </div>
      ...
      <div class="___relative" v-for="(item, index) in data.valueData.point">
        ...
        >
          ...
          <div class="___relative">
            ...
            <div
              class="___absolute Full borderRight"
              style="width: 350px;left:150px;"
            >
              <div class="___absolute Full borderRight" style="width: 175px;">
                <divModel
                  v-if="JudgeShow(item.valueChoice)"
                  v-model="item.value"
            ...
            <div class="___relative" style="width: 210px;left:505px;">
              ...
              <div v-if="valShow()" class="___relative ml5">
                <span>在规定值内:</span>
                <divModel
                  v-model="item.prescribed"
                  style="width:60px;text-align: left;left:70px;"
                  class="Full hide focusBg"
                ></divModel>
              </div>
  ...
  methods: {
    changeNum(valueChoice, value, detectionResult, index) {
      if (this.testingState === 1) {
        this.data.valueData.point[index].result = valueChoice
          ? this.contrast(value, detectionResult)
          : detectionResult;
        ...
        if (this.purposeDetection === "新标准首次检测") {
          ...
          if (this.isNumber(detectionResult) && this.isNumber(value)) {
            this.data.valueData.point[index].deviation = valueChoice
              ? (((detectionResult - value) / value) * 100).toFixed46(2) + "%"
              : "";
          }
        }
      }
