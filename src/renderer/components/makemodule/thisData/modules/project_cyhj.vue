<template>
	<div class="___relative">
		<div>
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point" :key="index">
					...
					<td>
						<myInput v-model="item.sampleName" style="text-align: center"></myInput>
					</td>
					<td>
						<divModel
										v-model="item.flow"
					...
					<td>
						<divModel v-model="item.volume" :edit="target === '0' && task.docPass !== 1"></divModel>
					</td>
					<td>
						<div class="___relative">
							<div
		...
		<el-dialog :visible.sync="delRowReason" width="50%" :modal="false" :title="'删除原因'">
			<el-input
							type="textarea"
