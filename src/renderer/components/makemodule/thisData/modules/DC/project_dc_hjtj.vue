				...
				<tr>
					...
					<td class="___relative tc" style="width:130px;">
						<div v-if="target == 0">
							...
							<myInput style="text-align: center" v-else v-model="data.valueData.weather"
											 @change.native="(el) => {noShowInput(el)}"></myInput>
						</div>
						<div v-else>{{data.valueData.weather}}</div>
					</td>
					<td class="___relative" style="width:210px;">
						<span class="ml5">温度：</span>
						<myInput style="text-align: center;width:50px;" v-model="data.valueData.temperature[0]"
										 :defaultValue="data.valueData.temperature[0]"></myInput>
						<span>至</span>
						<myInput style="text-align: center;width:50px;" v-model="data.valueData.temperature[1]"
										 :defaultValue="data.valueData.temperature[1]"></myInput>
						<span>℃</span>
					</td>
					<td class="___relative tc" style="width:220px;">
						<span class="ml5">湿度：</span>
						<myInput style="text-align: center;width:50px;" v-model="data.valueData.humidity[0]"
										 :defaultValue="data.valueData.humidity[0]"></myInput>
						<span>至</span>
						<myInput style="text-align: center;width:50px;" v-model="data.valueData.humidity[1]"
										 :defaultValue="data.valueData.humidity[1]"></myInput>
						<span>%（RH）</span>
					</td>
				</tr>
			</table>
