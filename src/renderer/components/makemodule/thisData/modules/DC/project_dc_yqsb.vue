				...
				<tr v-for="(item,index) in data.valueData.point">
					<td class="___relative tc">
						<div v-if="target == 0">
							...
							<myInput style="text-align: center" v-else v-model="item.rows[0]"
											 @change.native="(el) => {noShowInput(el, index)}"></myInput>
						</div>
						<div v-else>{{item.rows[0]}}</div>
					</td>
					<td class="___relative tc">
						<querySelect v-model="item.rows[1]"
												 v-if="target === '0'"
												 ref="querySelect"
												 :num="index"
												 :list="deviceData"
  ...
  export default {
    ...
    methods: {
      ...
      returnVal2(index, val) {
        ...
        this.data.valueData.point[index].deviceObj = JSON.stringify(val);
      },
    ...
    mounted() {
      console.log('仪器设备', this.deviceData)
    }
  };
</script>
