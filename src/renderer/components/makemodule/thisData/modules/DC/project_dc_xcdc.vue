<template>
	<div>
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<input type="file" :id="'project_dc_xcdc-filename'+pageNumber+'-'+thisPageIndex" accept="image/*"
						 @change="changFile"
						 style="display:none">
			...
			<table class="myTable">
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td class="___relative tc" style="width: 560px;" v-if=" target === '0'">
						<span :class="item.state===0 ? 'red':''">{{item.state | toDot}}</span>
						<div class="___absolute toolBar" v-if="item.title==='现场检测照片'" style="left: 635px;top:0; width: 60px;">
						...
						<div v-if="target === '0'" class="___absolute toolBar"
								 :style="item.title==='现场检测照片' ? {left:'700px'} : {left:'635px'}"
								 style="top:0; width: 120px;">
							<div title="上传图片" @click="upload(index,item.title)" class="___absolute tc">+</div>
							...
							<div title="缺失" @click="item.state=2" class="___absolute tc" style="left:90px;"><i
		...
		<el-dialog title="图片" :visible.sync="showImg" width="60%" :modal="false">
			<img v-if="showImg" style="max-width:80%;max-height:500px;" :src="hostUrl+data.valueData.point[currencyIndex].img"
					 alt="">
			<div slot="footer">
  ...
  export default {
    ...
    methods: {
      upload(index, title) {
        ...
        document.getElementById('project_dc_xcdc-filename' + this.pageNumber + '-' + this.thisPageIndex).click();
      },
      changFile() {
        let dataArr = [];
        ...
        let that = this;
        let filename = document.getElementById('project_dc_xcdc-filename' + this.pageNumber + '-' + this.thisPageIndex);
