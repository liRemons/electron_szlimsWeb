<template>
	<div>
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				<tr>
					<td rowspan="2" style="width:100px;line-height: 16px;">影像增强输入<br>屏视野尺(mm)</td>
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td>
						<myInput style="text-align: center" v-model="item.voltage" @change.native="changeNum(index)"
										 :defaultValue="item.voltage"></myInput>
					</td>
					<td>
					...
					<td>
						<myInput style="text-align: center" @change.native="changeNum(index)" v-model="item.val1"
										 :defaultValue="item.val1"></myInput>
					</td>
					<td>
						<myInput style="text-align: center" @change.native="changeNum(index)" v-model="item.val2"
										 :defaultValue="item.val2"></myInput>
					</td>
					<td>
						<myInput style="text-align: center" v-model="item.val3" @change.native="changeNum(index)"
										 :defaultValue="item.val3"></myInput>
					</td>
					<td class="hide">
  ...
  export default {
    ...
    methods: {
      ...
      changeNum(index) {
        ...
        if (this.isNumber(voltage) && this.data.valueData.factorArr instanceof Array && this.data.valueData.factorArr.length > 0) {
          this.data.valueData.point[index].factor = this.getFactor(voltage, this.data.valueData.factorArr)
        } else {
        ...
        let filterLine = this.data.valueData.point[index].filterLine;
        let arr = [E, F, G, H];
        let result = '';
        if (arr.every((item, index) => {
          return this.isNumber(item)
        })) {
          if (company === 'μGy/s' && filterLine === '有') {
            result = (((E + F + G) / 3) * H * 0.5 * 60).toFixed46(2);
          } else if (company === 'nGy/s' && filterLine === '有') {
            result = ((((E + F + G) / 3) * H * 0.5 * 60) / 1000).toFixed46(2);
          } else if (company === 'μGy/s' && filterLine === '无') {
            result = (((E + F + G) / 3) * H * 60).toFixed46(2);
          } else if (company === 'nGy/s' && filterLine === '无') {
            result = ((((E + F + G) / 3) * H * 60) / 1000).toFixed46(2);
          }
        }
      ...
      returnVal3(val, name, index) {
        this.data.valueData.point[index].filterLine = val;
        this.changeNum(index);
      },
    },
    computed: {
