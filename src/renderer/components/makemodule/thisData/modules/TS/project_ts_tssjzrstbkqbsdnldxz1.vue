	...
	<div>
		<div :class="{_normalHeight_:true}" class="___relative mt20">
			...
			<table class="myTable">
				...
				<tr class="tl">
					<td colspan="5">
						<span>验收&状态：≤50@透视荧光屏设备；验收&状态&稳定性：≤25@非透视荧光屏设备。</span>
					</td>
				</tr>
				<tr>
					<td colspan="5">
				...
				<tr class="tl">
					<td colspan="5" style="padding-left:10px;line-height: 24px;text-indent:25px;">
						<span>将尺寸为30cm×30cm×20cm的水模放置在探测器与影像接收器之间，调节至影像接收器最大照射野尺寸下<br>进行检测，检测条件见下表，计算公式：检测结果=(∑k)/3×f</span>
					</td>
				</tr>
				<tr>
					<td>透视设备类型</td>
				...
				<tr>
					...
					<td colspan="2">
						<myInput @change.native="changeNum()" @focus.native="ready()" style="text-align: center"
										 v-model="data.valueData.size"
										 :defaultValue="data.valueData.size"></myInput>
					</td>
				</tr>
				<tr>
					<td colspan="3">
					...
					<td colspan="2">
						<div v-if="target == 0">
							<selectModel @returnVal="returnVal"
													 ...
													 :single="true"
													 :rows="false"
													 :list="['影像增强器床上管', 'C形臂', '影像增强器床下管', '普通荧光屏透视', '自定义']"
													 :Obj="''">
							</selectModel>
						</div>
