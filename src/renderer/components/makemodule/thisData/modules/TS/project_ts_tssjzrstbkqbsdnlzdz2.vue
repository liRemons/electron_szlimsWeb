		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable">
				...
				<tr v-for="(item,index) in data.valueData.point">
					...
					<td>
						<myInput style="text-align: center" @change.native="changeNum(index)" v-model="item.voltage"
					...
					<td>
						<myInput style="text-align: center" @change.native="changeNum(index)" v-model="item.val3"
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        sizeList: state => state.StomatologyLinkage.sizeList2,
      })
    },
    watch: {
      deviceFactor(val) {
