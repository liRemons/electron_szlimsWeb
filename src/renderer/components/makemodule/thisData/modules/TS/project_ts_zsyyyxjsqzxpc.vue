	...
	<div>
		<div :class="{_normalHeight_:true}" class="___relative mt20">
			...
			<table class="myTable">
				...
				<tr class="tl">
					<td colspan="3" style="padding-left:10px;line-height: 24px;text-indent:25px;">
						<span>调节焦点至影像接收器之间距离最近，照射野设置为最大，将准直测试板放置在影像接收器上使影像接收器<br>中心与测试板中心对准，在透视条件下从显示器中观察并记录照射野中心到影像接收器中心距离，记录焦点至<br>影像接收器之间的距离SID，计算公式为：中心偏差=照射野中心到影像接收器中心距离/SID×100%。</span>
					</td>
				</tr>
				<tr>
					<td colspan="3">
				...
				<tr>
					<td>
						<myInput @change.native="changeNum()" style="text-align: center" v-model="data.valueData.view"
										 :defaultValue="data.valueData.view"></myInput>
					</td>
					<td>
						<myInput @change.native="changeNum()" style="text-align: center" v-model="data.valueData.SID"
										 :defaultValue="data.valueData.SID"></myInput>
					</td>
					<td>
