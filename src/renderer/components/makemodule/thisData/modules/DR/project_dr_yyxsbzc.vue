<template>
  ...
  >
    ...
    <div>
      ...
      <div
        style="border: 1px solid black; border-top: none; line-height: 20px; min-height: 31px;"
        class="tl"
      >
        {{ this.getContent(0) }}
      </div>
...
window.hh = {
  ...
  data() {
    return {
      content1:
        "采用直读法，选择80kV，如果不能选择80kV则尽可能选择相邻档中比80kV档低的一档，测器位置同管电压指示的偏离检测项目。",
      content2: "采用直读法，探测器位置同管电压指示的偏离检测项目。",
      content3:
        "探测器有效点位于乳房支撑台上方10cm处，将压迫器调至焦点与探测器之间约1/2处，选用28kV，合适的mAs，分别选择不同的靶/滤过进行曝光，采用直读法，读取半值层。",
      content4:
        "探测器有效点位于乳房支撑台上方10cm处，将压迫器调至焦点与探测器之间约1/2处，选用28kV，合适的mAs，靶/滤过为Mo/Mo，进行曝光，采用直读法，读取半值层。",
      list: [
        { name: "Mo/Mo", limitValue: [{ down: 0.3 }, { up: 0.4 }] },

        { name: "Mo/Rh", limitValue: [{ down: 0.3 }, { up: 0.47 }] },

        { name: "Rh/Rh", limitValue: [{ down: 0.3 }, { up: 0.5 }] },

        { name: "Rh/Al", limitValue: [{ up: 0.3 }] },

        { name: "W/Rh", limitValue: [{ down: 0.3 }, { up: 0.58 }] },

        { name: "W/Al", limitValue: [{ down: 0.3 }, { up: 0.53 }] },

        { name: "W/Ag", limitValue: [{ down: 0.3 }, { up: 0.6 }] }
      ],
      list2: [
        { name: "Mo/Mo", limitValue: [{ down: 0.3 }, { up: 0.4 }] },
        ...
        { name: "Rh/Al", limitValue: [{ down: 0.3 }] },

        { name: "W/Rh", limitValue: [{ down: 0.3 }] },

        { name: "W/Al", limitValue: [{ down: 0.3 }] },

        { name: "W/Ag", limitValue: [{ down: 0.3 }] }
      ]
    };
  },
  computed: {
    ...
    deviceType() {
      return this.jsonString.find(item => item.to === "project_jcxcxx").data
        .valueData.deviceType;
    },
