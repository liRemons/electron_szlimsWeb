<template>
  <div style="line-height: 30px; padding-top: 10px;" class="___relative">
    ...
    <div style="text-align: center;">
      ...
      <div style="border: 1px solid black; border-top: none;">
        <el-radio
          ...
          @click.native.prevent="
            computeObj.clearRadio('平均像素值', data.valueData, 'radioValue')
          "
      ...
      <table class="myTable" v-if="testType === '验收检测'">
          ...
          <td colspan="3">
            <div style="text-align: left; padding-left: 14px; padding: 5px;">
              <el-radio
                ...
                "
                label="有DDI值及生产厂家计算公式 计算公式："
              ></el-radio>
              <myInput
                v-if="hasDDI"
                v-model="data.valueData.computer"
        ...
        <tr>
          <td>
            <myInput
              v-if="hasDDI"
              v-model="data.valueData.arr2[0]"
              :reg="'[^0-9./]'"
              @change.native="setPianCha"
            ></myInput>
            <div v-else>/</div>
          </td>
          <td>
          ...
          <td colspan="3">
            <div style="text-align: left; padding-left: 5px;">
              <el-radio
                ...
                @click.native.prevent="
                  computeObj.clearRadio(
                    '无DDI值及生产厂家计算公式：建立基线值 =',
                    data.valueData,
                    'radioValue'
                  )
                "
                label="无DDI值及生产厂家计算公式：建立基线值 ="
              ></el-radio>
              <span v-if="!hasDDI" style="border-bottom: 1px solid black;">{{
                data.valueData.arr1[3]
              }}</span>
              <span v-else>/</span>
            </div>
      ...
      <table class="myTable" v-if="testType === '状态检测'">
        ...
        <tr>
          <td>
            ...
            <myInput
              v-else
              v-model="data.valueData.jiXianZhi"
              :reg="'[^0-9./]'"
              @change.native="
                el => {
                  setShowInput(el, 1);
          ...
          <td style="text-align: left; padding-left: 10px;">
            <p>
              ...
              <span v-if="data.valueData.checkArr[0]">{{
                data.valueData.arr1[3]
              }}</span>
            </p>
            <p>
              ...
              <span v-if="data.valueData.checkArr[1]"
                >{{ data.valueData.pianCha }}%</span
              >
            </p>
          </td>
        </tr>
      ...
      <div
        style="border: 1px solid black; border-top: none; line-height: 25px; text-align: left; "
      >
        <p>
          <el-radio
  ...
  methods: {
    setAv() {
      let sum = this.computeObj.sum(
        this.data.valueData.arr1[0],
        this.data.valueData.arr1[1],
        this.data.valueData.arr1[2]
      );
      let avg = sum / 3;
