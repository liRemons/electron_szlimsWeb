  ...
  methods: {
    ...
    setDisable() {
      ...
      if (this.computeObj.intersect(this.mode, arr1).length > 0) {
        ...
        this.computeObj.intersect(this.mode, arr3).length > 0 ||
        this.computeObj.intersect(this.mode, arr4).length > 0 ||
        this.computeObj.intersect(this.mode, arr5).length > 0
      ) {
        this.data.valueData.point[0].disabled = false;
        this.data.valueData.point[1].disabled = false;
    ...
    setAvg(point) {
      point.forEach(item => {
        item.v5 = (this.computeObj.sum(item.v2, item.v3, item.v4) / 3).toFixed(
          3
        );
      });
    },
