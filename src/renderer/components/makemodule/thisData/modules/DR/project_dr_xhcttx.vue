<template>
    <div style="line-height: 30px; padding-top: 10px;" class="___relative">
        ...
        <div style="text-align: center;">
            ...
            <table class="myTable">
                <tr class="delLine">
                    <td style="width: 100px; text-align: left; padding-left: 15px; padding: 5px;" colspan="4">
                        拟合公式：P=
                        <myInput v-model="data.valueData.gs1" style="width: 250px; border-bottom: 1px solid black;" :disabled="data.valueData.noteState !== '其他' || data.valueData.noteState === ''"></myInput>R²=
                        <myInput v-model="data.valueData.gs2" style="width: 100px; border-bottom: 1px solid black;" :disabled="data.valueData.noteState !== '其他' || data.valueData.noteState === ''"></myInput>
                    </td>
                </tr>
            </table>
            <div style="border: 1px solid black;font-weight: bolder; border-top: none;">
            ...
            <div style="border: 1px solid black; border-top: none; padding: 15px; text-align: left;">
                <el-radio v-model="data.valueData.noteState"
                          @click.native.prevent="computeObj.clearRadio('线性关系', data.valueData, 'noteState')"
                          label="线性关系"></el-radio>
                <el-radio v-model="data.valueData.noteState"
                          @click.native.prevent="computeObj.clearRadio('对数关系', data.valueData, 'noteState')"
                          label="对数关系"></el-radio>
                <el-radio v-model="data.valueData.noteState"
                          @click.native.prevent="computeObj.clearRadio('指数关系', data.valueData, 'noteState')"
                          label="指数关系"></el-radio>
                <el-radio v-model="data.valueData.noteState"
                          @click.native.prevent="computeObj.clearRadio('其他', data.valueData, 'noteState')"
                          label="其他"></el-radio>

                <myInput v-model="data.valueData.noteOther" style="width: 250px; border-bottom: 1px solid black; padding: 0" v-if="data.valueData.noteState === '其他'"></myInput>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        computed: {
            tcqData(){
                try {
                    return this.jsonString.find(item => item.to === 'project_dr_tcq').data.valueData;
                } catch (e) {
                    return ''
                }
            }
        ...
        methods: {
            ...
            lineFitting(x, y) { //分别为公式，R的平方值
                ...
                // y=ax+b
                this.data.valueData.gs1 = (this.isNumber(result[1].toFixed(4)) ? result[1].toFixed(4) : 0) + "* X " + (this.isNumber(s) ? s : 0);
                this.data.valueData.gs2 = this.isNumber(R2.toFixed(4)) ? R2.toFixed(4) : 0;
            },
            ...
            expFitting(x, y) {
                ...
                this.data.valueData.gs1 = (this.isNumber(A) ? A : 0) + "*e^(" + (this.isNumber(B) ? B : 0) + "*X)";
                this.data.valueData.gs2 = this.isNumber(R2.toFixed(4)) ? R2.toFixed(4) : 0;
            },
