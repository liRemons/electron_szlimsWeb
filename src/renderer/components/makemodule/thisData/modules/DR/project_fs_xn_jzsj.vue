<template>
  <div class="___relative" style="padding-top: 20px;">
    ...
    <table border="1" class="myTableReset _normalHeight_">
      ...
      <tr>
        <td rowspan="2" align="center">预设值</td>
        <td colspan="3" align="center">曝光时间测量值</td>
        <td rowspan="2" align="center">平均值</td>
        <td rowspan="2" align="center">偏离 / ms</td>
...
export default {
  data() {
    return {
      content1:
        "重点测量临床常用时间档进行检测，选择100ms以下测量1档，100ms及以上测量1档。偏差=曝光时间测量值-曝光时间预设值；相对偏差=（偏差/曝光时间预设值）×100%。 ",
      content2:
        "以设备常用成人曝光条件进行检测。偏差=曝光时间测量值-曝光时间预设值；相对偏差=（偏差/曝光时间预设值）×100%。",
      content3:
        "选择常用的曝光时间设置值，选择200ms以上测量1档，200ms及以下测量1档，读取曝光时间测量值。偏差=曝光时间测量值-曝光时间预设值；相对偏差=（偏差/曝光时间预设值）×100%。 ",
    };
  },
  props: [
    "ipdTemplate",
  ...
  methods: {
    ...
    changeNum(num1, num2, num3, num4, index) {
      ...
      let calculation = this.judgeNum(arr);
      let average = this.average(calculation).toFixed46(
  ...
  components: {},
  mounted() {
    this.data.valueData.unit ? "" : (data.valueData.unit = "ms");
  },
};
</script>
