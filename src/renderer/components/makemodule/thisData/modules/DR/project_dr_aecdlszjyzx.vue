        ...
        <div>
            ...
            <table class="myTable">
                <tr class="delLine">
                    ...
                    <td style="width: 90px;">检测结果<sub>23</sub></td>
                    <td style="width: 90px;">平均值偏差<sub>13</sub></td>
                </tr>
                <tr>
                    <td>
                        <el-radio label="mA、s" style="width: 30px;"
                                  @click.native.prevent="computeObj.clearRadio('mA、s', data.valueData, 'radioValue')"
                                  v-model="data.valueData.radioValue"></el-radio>
                        <el-radio label="mAs" style="width: 30px;"
                                  @click.native.prevent="computeObj.clearRadio('mAs', data.valueData, 'radioValue')"
                                  v-model="data.valueData.radioValue"></el-radio>
                        <el-radio label="DDI值" style="width: 30px;"
                                  @click.native.prevent="computeObj.clearRadio('DDI值', data.valueData, 'radioValue')"
                                  v-model="data.valueData.radioValue"></el-radio>
                    </td>
                    <td>
                        <myInput v-model="data.valueData.arr[0]" @change.native="control"></myInput>
                    </td>
                    <td>
                        <myInput v-model="data.valueData.arr[1]" @change.native="control"></myInput>
                    </td>
                    <td>
                        <myInput v-model="data.valueData.arr[2]" @change.native="control"></myInput>
                    </td>
                    <td>
...
<script>
    export default {
        ...
        methods: {
            ...
            gs1(){
                this.data.valueData.arr[3] = ((this.A - this.B) / (this.A + this.B)).toFixed(2);
            },
            gs2(){
                this.data.valueData.arr[4] = ((this.B - this.C) / (this.B + this.C)).toFixed(2);
            },
            gs3(){
                this.data.valueData.arr[5] = ((this.A - this.C) / (this.A + this.C)).toFixed(2);
            },
            sure(){
