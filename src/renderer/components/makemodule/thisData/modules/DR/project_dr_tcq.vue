    ...
    <div>
      <table class="myTable">
        <tr>
          ...
          <td>
            <myInput
              v-model="data.valueData.tcqNum1"
              v-if="data.valueData.NumberOfDetectors <= 2"
            ></myInput>
            <div v-else>/</div>
          </td>
          ...
          <td>
            <myInput
              v-model="data.valueData.tcqNum2"
              v-if="data.valueData.NumberOfDetectors > 1"
            ></myInput>
            <div v-else>/</div>
          </td>
        </tr>
      ...
      <div>
        <table class="myTable">
          <tr class="delLine" style="text-align: left;">
            <td
              style="padding-left: 10px; font-weight: bolder; border-right: none; width: 100px;"
            >
              曝光参数选择（探测器1）
            </td>
            <td
              style="border-right: none; border-left: none; width: 100px;"
            ></td>
            ...
            <td colspan="2" style="text-align: left; padding-left: 20px;">
              <el-radio
                ...
                @click.native.prevent="
                  computeObj.clearRadio('是', data.valueData, 'isPole')
                "
                v-model="data.valueData.isPole"
              ></el-radio>
              <el-radio
                label="否"
                @click.native.prevent="
                  computeObj.clearRadio('否', data.valueData, 'isPole')
                "
                v-model="data.valueData.isPole"
              ></el-radio>
            </td>
        ...
        <!-- 探测器2 -->
        <table class="myTable" v-if="data.valueData.NumberOfDetectors == 2">
          <tr class="delLine" style="text-align: left;">
            <td
              style="padding-left: 10px; font-weight: bolder; border-right: none; width: 100px;"
            >
          ...
          <tr>
            ...
            <td colspan="2" style="text-align: left; padding-left: 20px;">
              <el-radio
                label="是"
                @click.native.prevent="
                  computeObj.clearRadio('是', data.valueData, 'isPole2')
                "
                v-model="data.valueData.isPole2"
              ></el-radio>
              <el-radio
                label="否"
                @click.native.prevent="
                  computeObj.clearRadio('否', data.valueData, 'isPole2')
                "
                v-model="data.valueData.isPole2"
              ></el-radio>
            </td>
          ...
          <tr>
            ...
            <td colspan="2" style="padding-left: 10px;">
              <myInput
                v-model="data.valueData.cxbgcx2"
                style="text-align: left;"
              ></myInput>
            </td>
          </tr>
        ...
        <table class="myTable" v-if="data.valueData.NumberOfDetectors == 2">
          ...
          <tr v-for="(item, index) in data.valueData.point2" :key="index + '2'">
            <td>
              {{ item.v1 }}
            </td>
            <td>
            ...
            <td v-else-if="index === data.valueData.point2.length - 1">
              {{ item.v3 }}
            </td>
            <td v-if="index === 0" rowspan="5">
              <myInput v-model="item.v4"></myInput>
...
export default {
  ...
  mounted() {
    ...
    //   if (item.to == "project_dr_aecdlszjyzx") {
    //     console.log(index);
    //   }
    // });
  }
