<template>
  <div :class="{ _normalHeight_: true }" class="___relative">
    ...
    <div class="modules_1_tableBox ___relative mt10">
      ...
      >
        <p class="ml5 tl">
          验收：≥0.8@透视荧光屏设备，影像增强器透视设备：≥0.8@350mm(15in)，≥1.0@310mm(12in)，≥1.2@230mm(9in)，<br />≥1.4@150mm(6in)；状态：≥0.6；稳定性：基线值±20%内。
        </p>
      </div>
      ...
      >
        <p class="ml5">
          对普通荧光屏透视设备，将测试卡紧贴在荧光屏的入射面，以适当条件（如70kV，3mA）进行透视，从荧光屏上观察并记<br />录下能分辨的最大线对数。对于非普通荧光屏透视设备，检测量应将线对卡紧贴在影像接收器的入射屏或放在诊断床上，使<br />显示器中的测试卡的线条影像与扫描线的方向成45°夹角，以自动曝光控制条件或临床常用透视条件进行透视，如果出现影<br />像饱和现象（影像全白），可以在线对卡上放一块20mm厚铝板。从显示器上观察并记下能分辨的最大线对数。
        </p>
      </div>
      <p style="line-height: 16px;margin-top:10px;">
        4.4.3 检测条件、仪器示值与检测结果
      </p>
      <div
        class="___relative ___module_frame_Box"
      ...
      >
        ...
        >
          <div
            ...
            style="width: 33.3333%;height:32px;"
          >
            <divModel
              v-model="item.rows[0]"
...
export default {
  ...
  methods: {
    ...
    increase() {
      ...
      let index = nameArr
        .reverse()
        .findIndex((item, index) => item === "projcet_kjfbl");
      this.jsonString[
