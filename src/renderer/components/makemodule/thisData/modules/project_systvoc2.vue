	...
	<div>
		<div class="___relative" style="border:1px solid black;border-bottom:none">
			...
			<div class="___absolute t0" style="width:100px;left:80px;line-height:32px;">
				<divModel @change='synchronizationId' v-model="data.valueData.sysSolutionMultiple" :edit="target != '4'"
			...
			<tr v-for="(item, index) in data.valueData.point" :key="index">
				...
				<td v-if="showXieGan5(item) == 0">
					<divModel v-model="item.peakArea"
										...
										:computers="fengMianJi(data.valueData.allPoint[0].parallelWindArea1,'-A')">
					</divModel>
				</td>
				<td v-else-if="showXieGan5(item) == 1">
					<divModel v-model="item.peakArea"
										...
										:is-computer="true"
										:computerFormula="'gs10'"
										:computers="fengMianJi(data.valueData.allPoint[0].parallelWindArea2,'-B')">
					</divModel>
				</td>
				...
				<td v-if="showXieGan3(item) == 0">
					<divModel v-model="item.sysConcentration"
										...
										:computers="dataFactory(item,0,data.valueData.sysSolutionMultiple)"></divModel>
				</td>
				<td v-else-if="showXieGan3(item) == 1">
					<divModel v-model="item.sysConcentration"
										...
										:is-computer="true"
										:computerFormula="'gs15'"
										:computers="dataFactory(item,1,data.valueData.sysSolutionMultiple)"
					></divModel>
				</td>
				<td v-else>    <!--样品浓度-->
					<div>/</div>
				...
				<td v-if="showXieGan4(item) == 0" rowspan="2">    <!--报告值-->
					<divModel v-model="item.sysReport"
										:edit="false"
										:is-computer="true"
										:computerFormula="'gs15'"
				...
				<td v-if="showXieGan4(item) == 1 && showXieGan4(item) !== 3">     <!--报告值-->
					<div>/</div>
				</td>

				<td v-if="showXieGan4(item) == 0" rowspan="2">    <!--相对相差-->
					{{xiangCha(data.valueData.point, index)}}
				</td>
				<td v-if="showXieGan4(item) == 1 && showXieGan4(item) !== 3">    <!--相对相差-->
					<div>/</div>
				</td>
			</tr>
    ...
    methods: {
      ...
      dataFactory(item, num, xs) {
        ...
        try {
          ...
          let surplusData = point.find((item, index) => '其他（以甲苯计）' + (num === 0 ? '-A' : '-B') === item.testProject);
          ...
          sysSuckConcentration.forEach((item, index) => {
            let sysBlankConcentrationValue = Number(sysBlankConcentration[index]) > Number(item) ? 0 : Number(sysBlankConcentration[index]);
            sysSuckConcentrationResult += Number(item) - sysBlankConcentrationValue;
            let computered = this.nongDuComputer(item, sysBlankConcentrationValue, v8, t8, p8);
            val += computered;
          });
          let result = ((Number(sysSuckConcentrationResult) * 100) / (v8 * (273 / (273 + t8)) * (p8 / 101))) * xs;
      ...
      nongDuComputer(jie, blank, v, t, p) {
        ...
        let result = ((Number(jie) - Number(blank)) * 100) / (v * (273 / (273 + Number(t))) * (p / 101));
        result = eval(result);
        return result;
      },
      xiangCha(point, index) {
        ...
        let resultArr = this.computeObj.difference(sysConcentration1, sysConcentration2);
      ...
      showXieGan5(item) {
        let name = item.testProject;
        if (name.indexOf("TVOC总计-A") != -1 || name.indexOf("TVOC总计-B") != -1) {
          return 3;
