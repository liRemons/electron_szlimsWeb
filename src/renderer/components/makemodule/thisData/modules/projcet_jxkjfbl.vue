<template>
  <div class="___relative _normalHeight_">
    ...
    <div class="modules_1_tableBox ___relative mt10">
      ...
      >
        <p class="ml5">
          有可能的话取出滤线栅，设置SID等于或尽量接近180cm。选用两块分辨力测式卡，呈相互垂直的状态，放置于探测器表面<br />，选用适当条件（如用60kV，3mAs）进行曝光。验收检测建立基线值并与厂家保证值比较，无厂家保证值时与fNyquist比<br />较，并建立基线值；状态检测与基线值比较，对于厂家保证值为45°时的检测结果的，直接用厂家保证值除以1.414即为水平<br />和垂直方向的厂家保证值。
        </p>
      </div>
        ...
        >
          <div class="___relative borderBottom">
            ...
            <div
              ...
              style="width: 140px;left:90px;line-height: 16px;"
            >
              <span>最大可分辨线对<br />数目/lp/mm</span>
            </div>
            <div
              class="___absolute t0 tc borderRight"
              style="width: 140px;left:230px;line-height:16px;"
            >
              <el-checkbox
                v-model="item.manufactor"
            ...
            <div
              class="___absolute t0 tc borderRight"
              style="width: 150px;left:370px;"
            >
              <span>基线值/lp/mm</span>
            </div>
            <div
              class="___absolute t0 tc borderRight"
              style="width: 100px;left:520px;line-height:16px;"
            >
              <el-checkbox
                v-model="item.fNy"
                style="margin:0 5px 0 0;"
          ...
          <div
            class="___relative"
            :class="num != item.rows.length - 1 ? 'borderBottom' : ''"
          ...
          >
            ...
            <div
              class="___absolute Full t0 tc borderRight"
              style="width: 140px;left:230px;"
            >
              <divModel
                v-if="item.manufactor"
                v-model="val[2]"
                style="width:100%;text-align: center;"
                class="Full heightCenter3 hide focusBg"
              ></divModel>
            ...
            >
              <div v-if="JudgeVal()" class="___relative ml5">
                ...
                <divModel
                  ...
                  :is-computer="true"
                  :computers="
                    changeNum(val[1], val[2], val[4], val[5], index, num)
                  "
  ...
  methods: {
    changeNum(num1, num2, num4, num5, index, num) {
      ...
      if (this.testingState === 1) {
        if (
          this.data.valueData.point[index].manufactor &&
          this.isNumber(num2)
        ) {
          val = num2;
        } else if (
          this.data.valueData.point[index].fNy &&
          this.isNumber(num5)
        ) {
          val = num5;
        }
