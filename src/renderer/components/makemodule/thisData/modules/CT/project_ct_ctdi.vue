            ...
            <span style="white-space: normal; 20px !important">
              ...
              腰椎：＜35mGy。腹部：＜25mGy。
				...
				<tr>
					<td
									colspan="9"
									style="padding-left:10px;line-height: 20px;text-indent: 2em;text-align: left;"
					>
            <span
										style="white-space: normal; line-height:20px"
						>将模体置于扫描野中心，除头部使用头部模体外，其余部分均使用体部模体进行检测，选用临床常用条件对模体进行轴向扫描（头部和体部使用轴向扫描模式，腰椎和腹部可以使用螺旋扫描模式），记录仪器示值，根据公式：校准值=仪器示值平均值×校准因子/准直宽度，CTDIw=（模体中心校准值+2×模体四周校准值）/3，偏差=（CTDIw-厂家指标）/厂家指标×100%，对于螺旋扫描CTDIvol= CTDIw/螺距。</span>
					</td>
				</tr>
				<tr>
					<td colspan="9">
				...
				<template v-for="(item,index) in data.valueData.point">
					<tr>
						<td>管电压</td>
						<td>
							<myInput
						...
						<td>管电流</td>
						<td colspan="2">
							<myInput v-if="data.valueData.options.includes(index)" style="width: 50%;" v-model="item.b"
											 :defaultValue="item.b"></myInput>
							<selectModel
						...
						<td>曝光时间</td>
						<td>
							<myInput v-if="data.valueData.options.includes(index)"
											 style="text-align: center;width: 70%"
											 v-model="item.c"
											 :defaultValue="item.c"
							></myInput>
					...
					<tr>
						...
						<td>厂家指标</td>
						<td colspan="2">
							<myInput v-if="data.valueData.options.includes(index)" style="width: 50%;" v-model="item.f"
											 @change.native="reset()"
											 :defaultValue="item.f"></myInput>
							mGy
						</td>
						<td>CTDI 显示值</td>
						<td>
					...
					<tr style="white-space: normal;" v-for="(val, i) in item.datas" class="___relative" :key="index+''+i">
    ...
    methods: {
      ...
      changeNum(item, num, fuIndex) {
        switch (num) {
          case "a":
            if (item.a != "") {
              if (this.isNumber(item.a) && this.data.valueData.factorArr instanceof Array && this.data.valueData.factorArr.length > 0) {
          ...
          case "D":
            if (item.A != "" && item.B != "" && item.C != "") {
              item.D = ((Number(item.A) + Number(item.B) + Number(item.C)) / 3).toFixed46(1);
              item.E = ((item.D * 10) / Number(this.data.valueData.point[fuIndex].e)).toFixed46(1);
              item.F = (item.E * Number(this.data.valueData.point[fuIndex].h)).toFixed46(1);
            }
            if (this.data.valueData.point[fuIndex].datas.every((val, index) => {
              return this.isNumber(val.A) && this.isNumber(val.B) && this.isNumber(val.C)
            })) {
              let garr = this.data.valueData.point[fuIndex].datas.map(i => isNaN(i.F) ? 0 : Number(i.F));
              let temparr = this.data.valueData.point[fuIndex].datas;
              for (let i = 0; i < temparr.length; i++) {
                temparr[i].G = ((1 / 3) * garr[0] + (2 / 3) * ((garr[1] + garr[2] + garr[3] + garr[4]) / 4)).toFixed46(1);
              }
              let result = '';
