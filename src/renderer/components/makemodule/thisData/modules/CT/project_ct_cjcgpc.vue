		...
		<div :class="{_normalHeight_:true}" class="___relative">
            ...
            <span>
              ...
              <br/>稳定性：与基线值相差±20%或±1mm内，以较大者控制。
				...
				<tr class="tl">
					<td colspan="13" style="padding-left:10px;line-height: 20px;text-indent: 2em;">
            <span
				...
				<tr style="white-space: normal;" v-for="(item, index) in data.valueData.point" :key="index">
					<td colspan="2">{{item.scope}}</td>
					<td>
						<myInput :style="{color: (item.AColor?'red':'#000'), textAlign: 'center' }"
										 @change.native="changeNum(item,index,'A')"
										 v-model="item.A"
										 :defaultValue="item.A"></myInput>
					</td>
					<td>
					...
					<td>
						<myInput style="text-align: center"
										 ...
										 v-model="item.C"
										 :defaultValue="item.C"></myInput>
					</td>
					<td>
						<myInput :style="{color: (Number(item.D)<=Number(item.C)?'red':'#000'), textAlign: 'center' }"
										 style="text-align: center"
										 @change.native="changeNum(item,index,'E')"
										 v-model="item.D"
										 :defaultValue="item.D"></myInput>
					</td>
					<td>{{item.E}}</td>
					<td>
						<myInput style="text-align: center"
					...
					<td>
						<myInput style="text-align: center"
										 @change.native="changeNum(item,index,'J')"
										 v-model="item.G"
										 :defaultValue="item.G"></myInput>
					</td>
					<td>
						<myInput style="text-align: center"
										 @change.native="changeNum(item,index,'J')"
										 v-model="item.H"
										 :defaultValue="item.H"></myInput>
					</td>
					<td>
						<myInput style="text-align: center"
										 @change.native="changeNum(item,index,'J')"
										 v-model="item.I"
										 :defaultValue="item.I"></myInput>
					</td>
					<td>{{item.J}}</td>
					<td>{{item.K}}</td>
				</tr>
        ...
        switch (num) {
          ...
          case "E":
            if (this.isNumber(item.C) && this.isNumber(item.D)) {
              ...
              item.E = ((Number(item.C) + Number(item.D)) / 2).toFixed46(this.size([item.C, item.D]));
            }
            break;
          case "J":
            item.J =
