		...
		<div :class="{_normalHeight_:true}" class="___relative">
			...
			<table class="myTable" style="margin-bottom: 10px;">
				...
				<tr>
					<td colspan="7" style="text-align: left;padding-left: 10px;">
						<span>验收：＜0.35%；状态：＜0.45%；稳定性：与基线值相差±10%以内。</span>
					</td>
				</tr>
				<tr>
					<td colspan="7">
            ...
            <span
				...
				<tr>
					<td>SFOV</td>
					<td>
						<span>{{data.valueData.d}}</span>mm
					</td>
					<td>层厚</td>
					<td>
						<span :style="{ color: ((data.valueData.e<5||data.valueData.e>10)?'red':'#000')}">{{data.valueData.e}}</span>mm
					</td>
					<td>CTDI 显示值</td>
					<td>
						<span :style="{ color: (data.valueData.f>50?'red':'#000')}">{{data.valueData.f}}</span>mGy
					</td>
				</tr>
        ...
        switch (num) {
          case "D":
            if (Number(item.C) != "" || Number(item.B) != "")
              item.D = (Number(item.C) * Number(item.B)).toFixed46(1)*100/1000;
            break;
        }
      }
    },
