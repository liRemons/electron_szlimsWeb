  ...
  <div>
    ...
    <table class="myTable">
      <tr class="delLine">
        ...
        <td colspan="1" align="left" style="padding-left: 20px; border-right: 1px solid white; text-align: center">
          <divModel :is-num-box="true" :edit="edit" v-model="data.valueData.sysSolventDesorption"></divModel>
        </td>
    ...
    <table class="myTable" ref="ben">
      ...
      <tr v-for="(item, index) in data.valueData.point" :key="index">
        ...
        <td>{{item.volume}}</td>   <!--采样体积V（L）-->
        <td>{{item.temperature}}</td>   <!--采样点温度t（℃）-->
        <td>{{item.atmosphericPressure}}</td>   <!--采样点气压p（kPa）-->
        <td style="width: 140px;">{{item.myBlankSample && item.myBlankSample.length > 0 ?
        ...
        <td>{{item.sysBlankArea}}</td>   <!--空白峰面积-->
        <td>{{item.sysArea}}</td>   <!--样品峰面积-->

        <td v-if="target != 4">{{showNongDu(item)}}</td>
        <td v-else>{{item.sysConcentration}}</td>   <!--样品浓度-->
...
<script>
  ...
  export default {
    ...
    props: ['ipdTemplate', 'data', 'pageNumber', 'thisPageIndex', 'jsonString', 'showing', 'watchSign', 'isTemplate', 'ableInput', 'target', 'detectionLimitObj', 'bs', 'xieLv'],
    computed: {
      edit() {
        if (this.target != 4) {
          return true;
    ...
    methods: {
      ...
      showNongDu(item) {
        try {
          let sysBlankArea = Number(item.sysBlankArea);
          let sysArea = Number(item.sysArea);
          let xieLv = Number(this.xieLv);
          let v = Number(item.volume);
          let t = Number(item.temperature);
          let p = Number(item.atmosphericPressure);
          let x = Number(item.sysSolutionMultiple);
          let sysSolventDesorption = Number(this.data.valueData.sysSolventDesorption);
          if (sysSolventDesorption == '') sysSolventDesorption = 0;
          if (sysSolventDesorption !== 0) sysSolventDesorption = sysSolventDesorption / 100;
          let result2 = ((sysArea - sysBlankArea) / ((Number(xieLv) * v * (273 / (273 + t)) * (p / 101.3)) * sysSolventDesorption)) * x;
          result2 = result2.toFixed46(3, true, true);
