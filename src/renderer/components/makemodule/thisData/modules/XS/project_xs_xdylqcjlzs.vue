    ...
    <table class="myTable" ref="ben">
      ...
      <tr v-for="(item, index) in data.valueData.point" :key="index">
        ...
        <td class="___relative">
          <myInput
            v-if="item.sysSamplingQuantityUnit !== '件'"
            @change.native="changeNum(index)"
            style="width: 40px;"
            :class="
              (item.sysSamplingQuantityUnit === 'g' &&
                (item.sysSamplingQuantity < 1 ||
                  item.sysSamplingQuantity > 10)) ||
              (item.sysSamplingQuantityUnit === 'cm²' &&
                (item.sysSamplingQuantity < 1 ||
                  item.sysSamplingQuantity > 100))
                ? 'colorRed'
                : ''
            "
          ...
          <div
            ...
            style="width: 50px; left: 100px; line-height: 32px;"
          >
            <selectModel
              @returnVal="returnVal"
              :Judge="true"
  ...
  methods: {
    ...
    changeNum(index) {
      ...
      if (
        ...
        (sysSamplingQuantity < 1 || sysSamplingQuantity > 10)
      ) {
        this.err("采样量必须在1-10之内");
      } else if (
        sysSamplingQuantityUnit === "cm²" &&
        (sysSamplingQuantity < 1 || sysSamplingQuantity > 100)
      ) {
      ...
      if (
        this.isNumber(item1) &&
        this.isNumber(item2) &&
        this.isNumber(sysSamplingQuantity)
      ) {
        let result = "";
        if (item1 === 0 && item2 === 0) {
          if (sysSamplingQuantityUnit === "件") {
            result =
              10 / sysSamplingQuantity > 1
                ? "＜" +
                  (10 / sysSamplingQuantity).toFixed46(
                    this.sizeMin([sysSamplingQuantity]),
                    false,
                    true
                  )
                : "＜1";
