      ...
      <tr v-for="(item, index) in data.valueData.point" :key="index">
        ...
        <td class="___relative tl">
          <myInput
            ...
            :class="
              item.sysSamplingQuantityUnit === 'g' &&
              (item.sysSamplingQuantity >= 11 || item.sysSamplingQuantity <= 9)
                ? 'colorRed'
                : ''
            "
            :defaultValue="item.sysSamplingQuantity"
          ></myInput>
          <div
            class="___absolute t0"
            style="width: 50px; left: 50px; line-height: 32px;"
          >
            <selectModel
              @returnVal="returnVal"
              :Judge="true"
        ...
        <td>
          <myInput
            v-if="item.sysSamplingQuantityUnit != 'mL'"
            @change.native="changeNum(index)"
            :class="item.sysDiluentQuantity % 50 !== 0 ? 'colorRed' : ''"
            style="text-align: center;"
            v-model="item.sysDiluentQuantity"
            :defaultValue="item.sysDiluentQuantity"
          ></myInput>
          <span v-else>/</span>
        </td>
        <td>
...
export default {
  ...
  methods: {
    ...
    changeNum(index) {
      ...
      let sysSamplingQuantity = parseFloat(
        this.data.valueData.point[index].sysSamplingQuantity
      );
      let sysSamplingQuantityUnit = this.data.valueData.point[index]
        .sysSamplingQuantityUnit;
      let sysDiluentQuantity = this.data.valueData.point[index]
        .sysDiluentQuantity;
      sysDiluentQuantity = this.isNumber(sysDiluentQuantity)
        ? sysDiluentQuantity
        : 1;
      if (
        sysSamplingQuantityUnit === "g" &&
        (sysSamplingQuantityUnit >= 11 || sysSamplingQuantityUnit <= 9)
      ) {
        this.err("取样量必须在9.00-11.00之间");
      } else if (
        sysSamplingQuantityUnit === "g" &&
        sysDiluentQuantity % 50 !== 0
      ) {
        this.err("稀释液量必须是50的倍数");
      }
      let arr = [item1, item2, item3, item4, item5];
      if (
        [...arr, sysSamplingQuantity].every((item, index) => {
    ...
    returnVal(val, name, index) {
      ...
      this.data.valueData.point[index].sysUnit = arr.find((item, index) =>
        item.includes(val)
      );
      if (val.includes("mL")) {
