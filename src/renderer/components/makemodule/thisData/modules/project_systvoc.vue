				...
				<td colspan="3">
					<div>
						<el-checkbox-group v-model="data.valueData.testDeviceCheckBox">
							<el-checkbox :label="item.id" v-for="item of devices">{{item.deviceName}}</el-checkbox>
						</el-checkbox-group>
					</div>
				</td>
				<td>仪器型号</td>
		...
		<div v-for="(item, index) in data.valueData.point" :key="index">
			<table class="myTable" border="1" v-if="!item.isParallel">
				...
				<tr v-for="(item2, index2) in item.arr" :key="index2">
					...
					<!--峰面积-->
					<td v-if="index2 == 8">
						<divModel v-model="item2.peakArea"
											:edit="false"
											:is-computer="true"
											:computerFormula="'gs6'"
											:computers="[item.parallelWindArea, item.arr[0].peakArea, item.arr[1].peakArea, item.arr[2].peakArea, item.arr[3].peakArea, item.arr[4].peakArea, item.arr[5].peakArea, item.arr[6].peakArea, item.arr[7].peakArea]">
						</divModel>
					</td>
					...
					<!--解吸气浓度-->
					<td v-if="index2 == 8">
						<divModel v-model="item2.suckConcentration"
											:edit="false"
											:is-computer="true"
											:computerFormula="'gs7'"
											:computers="[regressionEquationValue1, regressionEquationValue2, regressionEquationValue3, item2.peakArea]">
						</divModel>
					</td>
					<td v-else-if="index2 > 8">
						<div>/</div>
					...
					<td v-else>
						<divModel v-model="item2.suckConcentration" :edit="false"></divModel>
					</td>

					<!--空白气浓度-->
					<td v-if="index2 == 8">
						<divModel v-model="item2.blankConcentration"
											:edit="false"
											:is-computer="true"
											:computerFormula="'gs7'"
											:computers="[regressionEquationValue1, regressionEquationValue2, regressionEquationValue3, item2.peakArea]"></divModel>
					</td>
					<td v-else-if="index2 > 8">
						<div>/</div>
					</td>
					<td v-else>
						<divModel v-model="item2.blankConcentration" :edit="false"></divModel>
					</td>

					<!--样品浓度-->
					<td v-if="index2 == 9">
						<divModel v-model="item2.sysConcentration"
											:edit="false"
											:is-computer="true"
											:computerFormula="'gs8'"
											:computers="[item.arr]">
					...
					<!--报告值-->
					<td v-if="index2 == 9">
						<divModel v-model="item2.sysReport"
											:edit="false"
											:is-computer="true"
											:computers="['42128', item.arr[0].peakArea, item.arr[1].peakArea, item.arr[2].peakArea, item.arr[3].peakArea, item.arr[4].peakArea, item.arr[5].peakArea, item.arr[6].peakArea, item.arr[7].peakArea, item.arr[8].peakArea]"
											:computerFormula="'gs2'"></divModel>
					</td>
					<td v-else>
						<div>/</div>
					</td>
			...
			<table class="myTable" border="1" v-else>
				...
				<tr v-for="(item2, index2) in item.arr" :key="index2">
					...
					<td v-else>
						<divModel v-model="item2.peakArea" :edit="false">
						</divModel>
					</td>
					...
					<td v-if="index2 == 17 || index2 == 16">
						<divModel v-model="item2.suckConcentration"
											...
											:computers="pingXingJieXi(item2.peakArea)"></divModel>
					</td>
					...
					<!--相对相差-->
					<td v-if="index2 == 18" rowspan="2">
						<divModel v-model="item2.sysDifference"
											:edit="false"
											:is-computer="true"
...
<script>
  ...
  export default {
    ...
    props: ['ipdTemplate', 'data', 'pageNumber', 'thisPageIndex', 'jsonString', 'showing', 'watchSign', 'isTemplate', 'ableInput', 'regressionEquationValue1', 'regressionEquationValue2', 'regressionEquationValue3', 'allFeng'],
    methods: {
      toShowTimeBox() {
      ...
      dataFactory2(computers, item) {
        try {
          ...
          sysSuckConcentration.forEach((item, index) => {
            sysSuckConcentrationResult += Number(item) - Number(sysBlankConcentration[index]);
            let computered = this.nongDuComputer(item, sysBlankConcentration[index], v8, t8, p8);
            val += computered;
          });
          let result = (Number(sysSuckConcentrationResult) * 100) / (v8 * (273 / (273 + t8)) * (p8 / 101));
      ...
      nongDuComputer(jie, blank, v, t, p) {
        ...
        let result = ((Number(jie) - Number(blank)) * 100) / (v * (273 / (273 + t)) * (p / 101));
        result = eval(result);
        return result;
      },
            ...
            if (item.isParallel) {
              ...
              tvocData.forEach(item => {
                if (item.sysSampleNum == labNum || item.sysSampleNum == labNum2) {
                  ...
                  let num = item.sysSampleNum.substr(item.sysSampleNum.length - 2, 2).trim();
                  name = name + num;
