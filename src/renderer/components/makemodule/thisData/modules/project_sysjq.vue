<template>
	<div :class="{_normalHeight_:data.height._normal.carried, _shortHeight_:!data.height._normal.carried}">
		<div :class="{eventCover:!ableInput}"></div>
		<projectHead v-if="headShow()" :data="data" :target="target" :col="10"></projectHead>
		<table style="width:100%" class="myTable">
			<tr class="delLine">
				<td style="width: 107px;">样品编号</td>
				<td style="width: 70px;">体积V(L)</td>
				<td style="width: 60px;">温度t(℃)</td>
				<td style="width: 70px;">气压p(kPa)</td>
				<td style="width: 119px;">样品溶液稀释倍数</td>
				<td style="width: 119px;">空白样品编号</td>
				<td style="width: 129px;">样品空白吸光度A0</td>
				<td style="width: 94px;">样品吸光度A</td>
				<td style="width: 124px;">样品浓度ρ(mg/m3)</td>
				<td style="width: 115px;">报告值ρ(mg/m3)</td>
				<td style="width: 80px;">相对相差(%)</td>
			</tr>
			<tr v-for="(item,index) in data.valueData.point" :key="index">
				...
				<td>
					<divModel v-model="item.volume" :edit="false" :is-num-box="true"></divModel>  <!--采样体积-->
				</td>
				<td>
					<divModel v-model="item.temperature" :edit="false" :is-num-box="true"></divModel>  <!--采样点温度-->
				</td>
				<td>
					<divModel v-model="item.atmosphericPressure" :edit="false" :is-num-box="true"></divModel>  <!--采样点气压-->
				</td>
				<td>
				...
				<td>
					<div>{{item.myBlankSample && item.myBlankSample.length > 0 ? item.myBlankSample[0].labSampleNum : ""}}</div>
					<!--空白样品编号-->
				</td>
				<td>
				...
				<td v-if="target != 4">
					<divModel :is-computer="true"
										:computerFormula="'gs1'"
										:computers="[item.volume, item.temperature, item.atmosphericPressure, item.sysBlankAbsorbance, item.sysAbsorbance, item.sysSolutionMultiple, bs, item,3]"
				...
				<td v-if="computeObj.toRowspan(item, index, data) && target != 4" rowspan="2"><!--报告值-->
					<div>
				...
				<td v-if="computeObj.toRowspan2(item, index) && target == 4"><!--报告值-->
					{{item.sysReport}}
				</td>
				<td v-if="computeObj.toRowspan2(item, index) && target == 4"><!--报告值-->
					/
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      name(item) {
        let name = `${item.sampleNum}${item.sampleNumIndex ? item.sampleNumIndex : ''}${item.parallelLetter ? item.parallelLetter : ''}`;
