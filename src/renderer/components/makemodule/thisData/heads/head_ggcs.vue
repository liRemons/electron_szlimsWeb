			...
			<div class="modules_1_tableBox ___relative">
				<table class="myTable">
					<tr>
						<td>受检单位名称</td>
						<td colspan="4">
							<myInput style="text-align: center" v-model="data.valueData.checkUnitName"></myInput>
						</td>
						<td>委托单位名称</td>
						<td colspan="5">
							<myInput style="text-align: center" v-model="data.valueData.clientUnitName"></myInput>
						</td>
					</tr>
					<tr>
						<td colspan="1">受检单位地址</td>
						<td colspan="10">
							<myInput style="text-align: center" v-model="data.valueData.checkUnitAddress"></myInput>
						</td>
					</tr>
					<tr>
						<td>检测对象</td>
						<td colspan="2">
							<myInput style="text-align: center" v-model="data.valueData.deviceTypeName"></myInput>
						</td>
						<td>评价依据</td>
						<td colspan="4">
							<myInput style="text-align: center" v-model="data.valueData.assess"></myInput>
						</td>
						<td>联系电话</td>
						<td colspan="2">
							<myInput style="text-align: center" v-model="data.valueData.checkUnitPhone"></myInput>
						</td>
					</tr>
					<tr style="height: 64px;">
						...
						<td
										colspan="10"
										style="text-align: left; padding: 4px; word-break: break-all;white-space: pre-wrap; "
				...
				<table class="myTable">
					<tr class="delLine" style=" height: 40px; line-height: 20px;">
						<td colspan="10" style="text-align: left;">
							<div class="___relative">
								<div class="___absolute" style="top: -8px; left: 18px; width: 80px;">现场调查</div>
								<div class="___absolute" style="top: -8px; left: 98px; width: 300px;">
									<div class="___relative">
										<div class="___absolute">（温湿度表仪器编号：</div>
										<div class="___absolute" style="left: 150px; border-bottom: 1px solid black;">
											<querySelect
								...
								<div class="___absolute" style="top: -8px; left: 418px; width: 100px;">
									<div class="___relative">
										<div class="___absolute">气压表仪器编号：</div>
										<div
														class="___absolute"
														style="left: 118px; width: 150px; border-bottom: 1px solid black;"
										>
											<querySelect
					...
					<tr
						...
						<td>
							<divModel v-if="target === '0'" v-model="poi.sampleAddress"></divModel>
							<span v-else>{{poi.sampleAddress | toDot}}</span>
						</td>
						<!--采样/检测地点   :reg="'[^0-9/]'"-->
						<td>
							<divModel v-if="target === '0'" v-model="poi.areaArea" :isNumBox="true"></divModel>
							<span v-else>{{poi.areaArea | toDot}}</span>
						</td>
						<!--区域面积m2-->
						<td>
							<divModel v-if="target === '0'" v-model="poi.temperature" :isNumBox="true"></divModel>
							<span v-else>{{poi.temperature | toDot}}</span>
						</td>
						<!--温度-->
						<td>
							<divModel v-if="target === '0'" v-model="poi.relativeHumidity" :isNumBox="true"></divModel>
							<span v-else>{{poi.relativeHumidity | toDot}}</span>
						</td>
						<!--湿度-->
						<td>
							<divModel v-if="target === '0'" v-model="poi.atmosphericPressure" :isNumBox="true"></divModel>
						...
						<td>
							<div v-if="target == 0">
								...
								<myInput
												...
												@change.native="(el) => {noShowInput(el, poi, 'showInput')}"></myInput>
							</div>
						...
						<td>
							<div v-if="target == 0">
								...
								<myInput
												...
												style="text-align: center"
												@change.native="(el) => {noShowInput(el, poi, 'showInput2')}"
								></myInput>
							</div>
						...
						<td>
							<div v-if="target == 0">
								...
								<myInput
												...
												style="text-align: center"
												@change.native="(el) => {noShowInput(el, poi, 'showInput3')}"
								></myInput>
							</div>
						...
						<td>
							<div v-if="target == 0">
								...
								<myInput
												...
												style="text-align: center"
												@change.native="(el) => {noShowInput(el, poi, 'showInput4')}"
								></myInput>
							</div>
